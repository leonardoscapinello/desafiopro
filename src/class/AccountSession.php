...
class AccountSession
{
    ...
    public function createSession($ignore_password = false)
    {
        global $database;
        global $security;
        if (not_empty($this->username) && not_empty($this->password)) {
            $database->query("SELECT id_account, password FROM accounts WHERE email = ?");
            ...
            if (count($resultset) > 0) {
                ...
                if ($db_password === $post_password || $ignore_password) {
                    return $this->storeSessionCookie($resultset[0]['id_account']);
                }
