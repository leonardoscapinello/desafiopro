<?php

class Accounts
{

    private $id_account;
    private $account_token;
    private $email;
    private $password;
    private $name;
    private $document;
    private $phone;
    private $insert_time;
    private $is_confirmed;
    private $personal_id;
    private $personal_points;
    private $id_coach;
    private $profile_picture;


    public function __construct($id_account = 0)
    {
        global $database;
        global $text;
        global $session;
        global $numeric;
        if (!$id_account || $id_account === null || $id_account === "") $id_account = $session->getIdAccount();
        if (not_empty($id_account)) {
            $database->query("SELECT * FROM accounts WHERE id_account = ? OR md5(id_account) = ?");
            $database->bind(1, $id_account);
            $database->bind(2, $id_account);
            $result = $database->resultsetObject();
            if ($result && count(get_object_vars($result)) > 0) {
                foreach ($result as $key => $value) {
                    $this->$key = $text->utf8($value);
                }
            }
        }
    }


    public function setCoachRequest($coach)
    {
        global $database;
        $database->query("SELECT * FROM accounts WHERE id_account = ? OR MD5(id_account) = ?");
        $database->bind(1, $coach);
        $database->bind(2, $coach);
        $result = $database->resultsetObject();
        if ($result && count(get_object_vars($result)) > 0) {
            return setcookie("SESS_COC", $coach, time() + (86400 * 30), "/");
        }
        return false;
    }

    public function getCoachRequest()
    {
        return get_request("SESS_COC");
    }

    public function resetPassword($new, $confirm_new)
    {
        global $database;
        global $numeric;
        global $security;
        global $session;
        try {

            if (strlen($new) > 5 && strlen($confirm_new) > 5) {
                if ($new === $confirm_new) {
                    if (notempty($this->getIdAccount())) {
                        if ($numeric->isIdentity($this->getIdAccount())) {
                            $security->setIdAccount($this->getIdAccount());
                            $pw = $security->hash($confirm_new, false);
                            $pw = $security->encrypt($pw);
                            $database->query("UPDATE accounts SET password = ? WHERE id_account = ?");
                            $database->bind(1, $pw);
                            $database->bind(2, $this->getIdAccount());
                            $database->execute();
                            $session->logoutFromAllSessions($this->getIdAccount());
                            return true;
                        }
                    }
                }
            }
        } catch (Exception $exception) {
            error_log($exception);
        }
        return false;
    }

    public function register($name, $email_address, $password, $phone = null)
    {
        global $database;
        global $security;
        try {

            $tk = new Token();
            $tkn = $tk::tokenNumeric(3) . "-" . $tk::tokenNumeric(3);

            $phone = preg_replace("/[^0-9]/", "", $phone);

            if (strlen($name) < 2) return -1;
            if (strlen($email_address) < 6) return -2;
            if (strlen($password) < 5) $password = null;
            if (strlen($phone) < 5) return -4;


            $database->query("SELECT email FROM accounts WHERE email = ?");
            $database->bind(1, $email_address);
            $r = $database->resultset();

            error_log("email" . $email_address);
            if (count($r) > 0) return 0;

            $id_coach = $this->getCoachRequest();
            $coach = new Accounts($id_coach);


            $database->query("INSERT INTO accounts (email, name, phone, account_token, id_coach) VALUES (?,?,?,?,?)");
            $database->bind(1, $email_address);
            $database->bind(2, $name);
            $database->bind(3, $phone);
            $database->bind(4, $tkn);
            $database->bind(5, $coach->getIdAccount());
            $database->execute();

            $id = $database->lastInsertId();
            if ($password !== null) {
                $tmp_account = new Accounts($id);
                $tmp_account->resetPassword($password, $password);
            }

            if ($id > 0) {
                $this->notifyNewAccount($name, $email_address, $tkn, $id);
            }

            return $id;

        } catch (Exception $exception) {
            error_log($exception);
        }
        return 0;
    }

    private function notifyNewAccount($name, $email, $token, $id_account)
    {
        global $emailNotification;
        try {
            $url = SITE_URL . "cadastro/confirmar?u=" . base64_encode($id_account) . "&t=" . base64_encode($token);
            $emailNotification->sender("Samuel");
            $emailNotification->contact($name, $email);
            $emailNotification->subject("Olá " . $name . ", falta bem pouco pra concluirmos, ok?");
            $emailNotification->paragraph("Olá, Samuel aqui,");
            $emailNotification->paragraph("Sou responsável pelo DesafioPro e vou te ajudar a concluir seu cadastro, combinado? ");
            $emailNotification->paragraph("Para garantir a integridade e segurança de todos os participantes, preciso que você confirme seu endereço de e-mail.");
            $emailNotification->paragraph("Seu código de verificação é: " . $token);
            $emailNotification->paragraph("Ou, se preferir você pode clicar no botão abaixo para seguirmos.");
            $emailNotification->button("Confirmar meu E-mail", $url);
            $emailNotification->paragraph("Se por alguma razão, não for possível clicar no botão, cole esse endereço na barra de endereço do seu navegador.");
            $emailNotification->paragraph($url);
            $emailNotification->send();
        } catch (Exception $exception) {
            error_log($exception);
        }
    }

    private function notifyConfirm()
    {
        global $emailNotification;
        try {
            $url = SITE_URL . "login?e=" . base64_encode($this->getEmail());
            $emailNotification->sender("Samuel");
            $emailNotification->contact($this->getName(), $this->getEmail());
            $emailNotification->subject("Uma mensagem para você, do criador do DesafioPro Brasil");
            $emailNotification->paragraph("Olá, Samuel aqui,");
            $emailNotification->paragraph("Estou escrevendo essa mensagem para te parabenizar por essa decisão incrível e dizer que você pode contar comigo como forte aliado nesse momento.");
            $emailNotification->paragraph("Seu acesso à area de membros já foi liberado e pra acessar, o endereço continua o mesmo: " . SITE_URL);
            $emailNotification->paragraph("Só uma observação: Seu acesso foi liberado mas pode ser que nossa equipe tenha de validar algumas informações mais a fundo com seu coach direto. Não se preoupe, se esse for o caso, um de nossos analistas já deve estar tomando conta disso pra ir o mais rápido possível.");
            $emailNotification->paragraph("De qualquer forma, você já pode acessar a duas aulas sobre <b>atividade física em casa</b> enquanto trabalhamos nisso.");
            $emailNotification->send();
        } catch (Exception $exception) {
            error_log($exception);
        }
    }


    public function confirm($token)
    {
        $d = new Database();
        try {
            $d->query("UPDATE accounts SET is_confirmed = 'Y' WHERE id_account = ? AND account_token = ?");
            $d->bind(1, $this->getIdAccount());
            $d->bind(2, $token);
            $d->execute();
            $r = $d->rowCount();

            $this->notifyConfirm();

            return true;
        } catch (Exception $exception) {
            error_log($exception);
        }
        return false;
    }


    public function getAllAffiliates()
    {
        $d = new Database();
        try {
            $d->query("SELECT * FROM accounts WHERE id_coach = ?");
            $d->bind(1, $this->getIdAccount());
            $d->execute();
            $result = $d->resultset();
            if (count($result) > 0) {
                return $result;
            }
        } catch (Exception $exception) {
            error_log($exception);
        }
        return array();
    }

    /**
     * @return mixed
     */
    public function getIdAccount()
    {
        return $this->id_account;
    }

    /**
     * @return mixed
     */
    public function getAccountToken()
    {
        return $this->account_token;
    }


    /**
     * @return mixed
     */
    public function getEmail()
    {
        return $this->email;
    }

    /**
     * @return mixed
     */
    public function getPassword()
    {
        return $this->password;
    }

    /**
     * @return mixed
     */
    public function getName()
    {
        return $this->name;
    }

    /**
     * @return mixed
     */
    public function getDocument()
    {
        return $this->document;
    }

    /**
     * @return mixed
     */
    public function getPhone()
    {
        return $this->phone;
    }

    /**
     * @return mixed
     */
    public function getInsertTime()
    {
        return $this->insert_time;
    }

    /**
     * @return mixed
     */
    public function isConfirmed()
    {
        return $this->is_confirmed === "Y" ? true : false;
    }

    /**
     * @return mixed
     */
    public function getPersonalPoints()
    {
        return intval($this->personal_points);
    }

    /**
     * @return mixed
     */
    public function getIdCoach()
    {
        return $this->id_coach;
    }

    /**
     * @return mixed
     */
    public function getProfilePicture()
    {
        return not_empty($this->profile_picture) ? $this->profile_picture : "data:image/png;base64,";
    }

    /**
     * @return mixed
     */
    public function getPersonalId()
    {
        return $this->personal_id;
    }

    public function updateAudit($id, $personal_id, $personal_point)
    {
        global $database;
        global $text;
        try {
            $database->query("UPDATE accounts SET personal_id = ?, personal_points = ? WHERE id_account = ?");
            $database->bind(1, $personal_id);
            $database->bind(2, $personal_point);
            $database->bind(3, $text->base64_decode($id));
            $database->execute();
            return true;
        } catch (Exception $exception) {
            error_log($exception);
        }
        return false;
    }


}
